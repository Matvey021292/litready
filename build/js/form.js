...
var Form = new function () {
    ...
    this.init = function () { 
        ...
        this.messages = {
            ...
            "age": "Пожалуйста, подтвердите ваше согласие с условиями",
    ...
    this.validationOnBlur = function () {
        ...
        this.form_container.find('input').blur( function(event){
            ...
            switch(validType)
            {
                ...
                {
                    $target.removeClass('valid').addClass('invalid');
                    $target.next('.error').html(Form.messages.phone)
                }
                break;

                case 'age':
                if( !$target.prop("checked") ){
                    $target.siblings('.error').html(Form.messages.age);
                }else{
    ...
    this.submitForm = function () {
        ...
        this.form_container.submit(function(e){
            ...
            var json = {
                ...
                phone: $(this).find('input[data-validation-type="countrycode"]').val().slice(1) + $(this).find('input[data-validation-type="phone"]').val(),
                countryISO: $(this).find('input[name="country"]').attr("code"),
    ...
    this.initCountrySelect = function () {
        ...
        $('input[name="country"]').countrySelect({
            ...
            defaultCountry: window.navigator.language.slice(0,2)
        });
...
Plugin.prototype = {
    ...
    _generateMarkup: function() {
        // Country input
        this.country_input = $(this.element);
        // containers (mostly for positioning)
        var mainClass = "country-select";
        if (this.options.defaultStyling) {
            mainClass += " " + this.options.defaultStyling;
        }
        this.country_input.wrap($("<div>", {
            "class": mainClass
        }));
        var flagsContainer = $("<div>", {
            "class": "flag-dropdown"
        }).insertAfter(this.country_input);
        ...
        this.countryCodeInput = $("#"+this.country_input.attr("id")+"_code");
        if (!this.countryCodeInput) {
            this.countryCodeInput = $('<input type="hidden" id="'+this.country_input.attr("id")+'_code" name="'+this.country_input.attr("name")+'_code" value="" />');
            this.countryCodeInput.insertAfter(this.country_input);
        }
    ...
    _setInitialState: function() {
        ...
        if (!flagIsSet) {
            ...
            this.country_input.attr('code', defaultCountry.iso2);
        }
    },
    ...
    _initListeners: function() {
        ...
        this.country_input.on("keyup" + this.ns, function() {
        ...
        selectedFlag.on("click" + this.ns, function(e) {
            ...
            if (that.countryList.hasClass("hide") && !that.country_input.prop("disabled")) {
                that._showDropdown();
            }
        });
        ...
        this.country_input.on("blur" + this.ns, function() {
            if (that.country_input.val() != that.getSelectedCountryData().name) {
                that.setCountry(that.country_input.val());
            }
            that.country_input.val(that.getSelectedCountryData().name);
        });
    },
    ...
    _focus: function() {
        this.country_input.focus();
        var input = this.country_input[0];
        // works for Chrome, FF, Safari, IE9+
        if (input.setSelectionRange) {
            var len = this.country_input.val().length;
    ...
    _setDropdownPosition: function() {
        var inputTop = this.country_input.offset().top, windowTop = $(window).scrollTop(),
        dropdownFitsBelow = inputTop + this.country_input.outerHeight() + this.dropdownHeight < windowTop + $(window).height(), dropdownFitsAbove = inputTop - this.dropdownHeight > windowTop;
    ...
    _updateFlagFromInputVal: function() {
        ...
        var value = this.country_input.val().replace(/(?=[() ])/g, '\\');

        if (value) {
            var countryCodes = [];
            var matcher = new RegExp("^"+value.slice(1), "i");
            ...
            if (!alreadySelected) {
                ...
                this._getCode(countryCodes[0], this.countryCodeInput);
            }
    ...
    _selectListItem: function(listItem) {
        ...
        this.country_input.trigger("change");
        this.countryCodeInput.trigger("change");
        // focus the input
        this._focus();
        this.country_input.attr('code', countryCode);
    },
    ...
    _updateName: function(countryCode) {
        ...
        this.country_input.val(this._getCountryData(countryCode).name);
        this._getCode(countryCode, $("input[name='countrycode']"));
    ...
    setCountry: function(country) {
        this.country_input.val(country);
        this._updateFlagFromInputVal();
    },
    // remove plugin
    destroy: function() {
        // stop listeners
        this.country_input.off(this.ns);
        this.selectedFlagInner.parent().off(this.ns);
        // remove markup
        var container = this.country_input.parent();
        container.before(this.country_input).remove();
    }
};
